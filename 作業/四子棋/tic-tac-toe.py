...
def alpha_beta(board, player, alpha, beta, depth):
    # 檢查終止條件
    ...
    for i in range(6):
        for j in range(6):
            if board[i][j] == '-':
                ...
                if player == 'X':
                    best_score = max(best_score, alpha_beta(board, 'O', alpha, beta, depth-1))
                    alpha = max(alpha, best_score)
                else:
                    best_score = min(best_score, alpha_beta(board, 'X', alpha, beta, depth-1))
...
def get_next_move(board, player):
    ...
    for i in range(6):
        for j in range(6):
            if board[i][j] == '-':
                board[i][j] = player
                score = alpha_beta(board, 'O' if player == 'O' else 'X', float("-inf"), float("inf"), 3)
                board[i][j] = '-'
...
def main():
    ...
    while has_empty(board) and not check_win(board, computer_player) and not check_win(board, real_player):
        ...
        if turn == real_player:
            ...
            print("電腦的回合。")
            # 呼叫alpha-beta函數進行搜索
            i, j = get_next_move(board, real_player)
